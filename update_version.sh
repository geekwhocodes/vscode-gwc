...
URL_BASE=https://github.com/geekwhocodes/vscode-gwc/releases/download/${LATEST_MS_TAG}
...
cd versions
git config user.email "ganeshraskar@fake-it.o"
git config user.name "gwc-vscode CI"
...
if [[ "$OS_NAME" == "osx" ]]; then
  ...
  ASSET_NAME=vscode-gwc-darwin-${LATEST_MS_TAG}.zip
  VERSION_PATH="darwin"
  JSON="$(generateJson ${ASSET_NAME})"
  updateLatestVersion "$VERSION_PATH" "$JSON"
elif [[ "$CI_WINDOWS" == "True" ]]; then
  # system installer
  ASSET_NAME=vscode-gwcSetup-${BUILDARCH}-${LATEST_MS_TAG}.exe
  VERSION_PATH="win32/${BUILDARCH}/system"
  JSON="$(generateJson ${ASSET_NAME})"
  updateLatestVersion "$VERSION_PATH" "$JSON"

  # user installer
  ASSET_NAME=vscode-gwcUserSetup-${BUILDARCH}-${LATEST_MS_TAG}.exe
  VERSION_PATH="win32/${BUILDARCH}/user"
  JSON="$(generateJson ${ASSET_NAME})"
  updateLatestVersion "$VERSION_PATH" "$JSON"

  # windows archive
  ASSET_NAME=vscode-gwc-win32-${BUILDARCH}-${LATEST_MS_TAG}.zip
  ...
  # as examples
  ASSET_NAME=vscode-gwc-linux-${VSCODE_ARCH}-${LATEST_MS_TAG}.tar.gz
